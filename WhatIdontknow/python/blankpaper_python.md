...
#### 1 함수(Functions)
특정 행동을 정해놓은 코드 집합 
한 번 만들어놓으면 계속 호출해 쓸 수 있음
def calculate_store_sale(): (동사_형용사_명사 순으로 직관적 정의)
#### 1-2 내장함수(built-in functions)
파이썬에 기본적으로 있는 유용한 함수 
리스트 상대 max, min, sum, sorted(, reverse = True), len등
#### 2 매개변수와 인자
parameter: 함수가 받아올 변수 설정-어떻게 명명해도 상관없으나 알아보기 쉽게끔 작성
위치 변수/ 기본값 변수(Value Default Parameter)- name = "홍길동" 처럼
.get()등으로 호출했을 때 기본적으로 나오는 변수/ *args한 번에 여러 위치 인자를 받아옴/ **kwargs여러 키워드 인자
순서 => 위치, 키워드, 다중 위치, 다중 키워드 순서 바꾸면 컴퓨터가 혼동함
#### 2-1 위치인자, 키워드 인자, 가변 인자
인자 - 실제로 함수에 전달되는 값 
위치인자는 기본적으로 정해진 곳에 투하, 키워드는 어디든 지정한 데 박힘
#### 3 재귀함수(Recursion)
스스로 자기 자신을 불러오는 함수
함수 내 종료 조건(base code)을 정해야 함
#### 4 스코프
변수의 위치 참조 순위? local -> enclsoed -> global -> built_in
global 변수로 안쪽에서 함수 전역에 변수를 적용할 수 있음
#### 5 패킹
함수를 정의할 때 *를 앞에 둬서 들어오는 여러 인자값을 튜플로 묶음/ 그냥 나열하면 알아서 튜플로 묶임
/ **를 앞에 둬서 들어오는 키워드 변수들을 딕셔너리 형태로 묶는 게 패킹
#### 5-2 언패킹
묶어놨다 각각의 변수에 다시 풀어내는 언패킹
기본 - 튜플은 변수 수를 일치시켜서/ 함수 호출할 때 func(*arg)식으로 매개변수에 쪼개서 집어던짐/ **매개변수명
#### 6 람다 함수
lambda 매개변수 : 표현식 ->임시함수 ex- lambda x, y: x + y
#### 7  지연함수(map, zip, range)
map - 게으른 요리사 
바로 음식을 만드는 게 아니라 레시피 형태로
저장해놓았다가 요청하면 만들어 주는 느낌 - for문으로 반복해 꺼내거나 리스트로 꺼내기  

### Python_Modules_Control_Of_Flow

#### 1 모듈
유용한 기능들을 모은 코드 덩어리. import로 가져올 수 있고 from import로 명시적 호출 가능. 책 하나
#### 1-2 패키지
모듈의 집합체. PSL(Python Standard Library)같은 기초 내장 패키지가 있으며 개발자들이 만들어놓은
패키지들을 명령어를 사용하여 불러올 수 있음 ex- .requests, .json 등. 책장
#### 1-3 라이브러리 
패키지 모음. 자주 쓰는 기능들을 공유하는 곳이 있음. ex- pip matplotlib 도서관 
#### 2 제어문
코드의 흐름을 제어하는 표현식이나 문장들
#### 3 조건문
if-elif-else문 코드 블럭의 흐름에 조건을 달아 멈추거나 조절 가능
if는 각각의 조건을 검사, elif는 if 문의 조건을 만족 못했을 때 작동, else는 예외 발생 시 
#### 4 반복문
반복 가능한 요소들(iterable)을 돌며 데이터를 찾거나 나열할 수 있게끔 하는 문법
list, tuple, 문자열뿐만 아니라 비시퀀스형 자료 set, dict도 적용가능(단, 순서는 보장할 수 없음)
#### 4-1 for반복문
for x in data 형태로 좌측 반복변수에 우측 반복 요소들에서 값 하나씩을 꺼내 넣고 아래 코드블록을
실행한다. range로 특정 횟수 반복 등 일정 요소를 반복할 때 사용 가능. break로 즉시 반복문 종료 후 값 돌려주기
continue로 해당 회차 반복만 건너뛰기. for else문으로 break의 모든 조건을 뚫었을 때 반환값 찾기 가능
#### 4-2 while반복문
while x 조건: 형태의 반복문. 조건을 만족(=True)인 동안에만 아래 코드블록을 실행한다. 반드시 종료 조건을 설정해야 
스택 오버플로우를 막을 수 있음(코드블록 내에서 조건을 향해 움직이는 조건). break 사용
#### 5 유용한 내장함수 map, zip, enumerate
map(함수, 반복대상) 형태로 반복대상 모두에 함수를 적용해 map????형태로 반환함. 이걸 튜플이나 리스트로 풀어낼 수 있음.
zip(반복대상1, 반복대상2...)은 반복대상의 같은 위치에 있는 요소들을 튜플로 묶어 zip????형태로 반환함. 이하 동일.
2차원 리스트에 적용하면 전치행렬을 순회하는 결과를 얻을 수 있음