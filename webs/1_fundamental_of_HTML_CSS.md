...
-Question
10min * 1
inline코드는 왜 다른 코드와 다르지?
레이아웃이 왜 필요하지?
z-???가 왜 클수록 다른 요소보다 위에 올라오지??
CSS를 왜 여러 요소로 나누어 적용하는 거지?
반응형 레이아웃이 왜 이해가 안 되지?
flowbox를 왜 쓰지?
노멀 플로우말고 다른 플로우는 왜 쓰는 거지?
왜 코드를 봐도 이게 어떻게 작동하는지 안 그려지지?
normal/absolute/fixed말고 다른 건 왜 쓰지?
왜 블록 개념과 형광펜 개념이 구별되는 거지?
<div>는 왜 블록을 구분하지?
